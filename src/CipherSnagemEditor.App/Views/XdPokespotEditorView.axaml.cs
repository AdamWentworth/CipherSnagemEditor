...
public partial class XdPokespotEditorView : UserControl
{
    ...
    private void AnimatePokemonBodies(object? sender, EventArgs e)
    {
        ...
        {
            if (!image.IsVisible
                || image.DataContext is not XdPokespotEntryViewModel pokespot
                || pokespot.BodyFrames.Count == 0)
            {
                continue;
            }

            var frame = SelectFrame(pokespot.BodyFrames, _spriteClock);
