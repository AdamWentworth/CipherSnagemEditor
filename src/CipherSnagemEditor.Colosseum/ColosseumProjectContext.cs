...
public sealed partial class ColosseumProjectContext
{
    ...
    private readonly Dictionary<string, byte[]> _standaloneLoadedFiles = new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, FsysArchive> _standaloneLoadedFsys = new(StringComparer.OrdinalIgnoreCase);
    private readonly Dictionary<string, GameStringTable> _standaloneLoadedStringTables = new(StringComparer.OrdinalIgnoreCase);
    private readonly GameCubeIsoWorkspace? _isoWorkspace;
        ...
        ColosseumSettings settings)
    {
        ...
        Settings = settings;
        _isoWorkspace = iso is not null && workspaceDirectory is not null
            ? new GameCubeIsoWorkspace(iso, workspaceDirectory, settings.IncreaseFileSizes)
            : null;
    }
    ...
    public Dictionary<string, byte[]> LoadedFiles => _isoWorkspace?.LoadedFiles ?? _standaloneLoadedFiles;

    public Dictionary<string, FsysArchive> LoadedFsys => _isoWorkspace?.LoadedFsys ?? _standaloneLoadedFsys;

    public Dictionary<string, GameStringTable> LoadedStringTables => _isoWorkspace?.LoadedStringTables ?? _standaloneLoadedStringTables;

    public ColosseumCommonRel? CommonRel { get; private set; }

    private IReadOnlyDictionary<int, string>? TrainerModelNames { get; set; }

    private GameCubeIsoWorkspace IsoWorkspace
        => _isoWorkspace ?? throw new InvalidOperationException("No Colosseum ISO workspace is available.");
    ...
    private string ResolveIsoExtractPath(string fileName, string? outputPath)
        => IsoWorkspace.ResolveIsoExtractPath(fileName, outputPath);
